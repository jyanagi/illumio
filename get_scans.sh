...
label_dimension=$(curl -s -k -X GET https://$pce_url/api/v2/orgs/$org_id/label_dimensions -u $ilo_api:$ilo_secret | jq -r '.[] | .display_name')
if $label_dimension | grep -q "Risk"; then
  ...
  curl -s -k -X POST https://$pce_url/api/v2/orgs/$org_id/label_dimensions -u $ilo_api:$ilo_secret -H "Content-Type: application/json" -d '{"key":"RI","display_name":"Risk","display_info":{"icon":"critical","initial":"RI","background_color":"#ff0000","foreground_color":"#ffffff","display_name_plural":"Risk"}}'
  curl -s -k -X POST https://$pce_url/api/v2/orgs/$org_id/labels -u $ilo_api:$ilo_secret -H "Content-Type: application/json" -d '{"key":"RI","value":"Risky"}'
  label=$(curl -s -k -X GET https://$pce_url/api/v2/orgs/$org_id/labels -u $ilo_api:$ilo_secret | jq '.[] | select(.value == "Risky") | .href')
fi
...
if [[ $selection =~ ^[0-9]+$ && $selection -ge 1 && $selection -le ${#scans[@]} ]]; then
  ...
  echo "Parsing... please wait as this process may take a few moments to complete"
